...
```powershell
# ======================================================================
# 模拟 Git 多分支开发场景：main + 3 个功能分支
# 目标：
#   - feature/user-auth     → 已合并到 main
#   - feature/notification  → 已合并到 main
#   - feature/payment       → 未合并（用于测试“未合并分支保护”）
# 注意：所有功能分支都从 main 独立分出，避免意外依赖
# ======================================================================
...
# ------------------------------------------------------------------
# 第 1 步：创建并合并 user-auth 功能
# ------------------------------------------------------------------
git checkout -b feature/user-auth          # 从 main 创建新分支

# 创建 src 目录（PowerShell 不会自动创建父目录！）
mkdir -Force src

# 写入新文件（这是 untracked 文件，必须显式 git add）
echo "auth code" > src/auth.ts

# 关键：新文件必须先 add，-a 才能生效
git add src/auth.ts
...
# ------------------------------------------------------------------
# 第 2 步：创建 payment 功能（但不合并！用于测试“未合并分支”）
# ------------------------------------------------------------------
git checkout main                          # 切回main，确保从 main 出发
git checkout -b feature/payment            # 独立分支

mkdir -Force src
echo "payment code" > src/payment.ts
git add src/payment.ts                     # 必须 add 新文件！
git commit -m "Add payment feature"

# ⚠️ 注意：这里不合并！保持 feature/payment 未集成到 main

# ------------------------------------------------------------------
# 第 3 步：创建并合并 notification 功能（也从 main 独立分出）
# ------------------------------------------------------------------
git checkout main                          # 切回 main（关键！避免从 payment 分支分出）
git checkout -b feature/notification       # 从干净的 main 创建
...
# 验证最终状态
# ------------------------------------------------------------------
Write-Host "`n✅ 当前 main 分支包含的文件：" -ForegroundColor Green
Get-ChildItem -Recurse | Where-Object { !$_.PSIsContainer } | ForEach-Object { $_.FullName }

Write-Host "`n✅ 已合并的分支（应包含 user-auth 和 notification）：" -ForegroundColor Green
git branch --merged

Write-Host "`n⚠️ 未合并的分支（应包含 payment）：" -ForegroundColor Yellow
git branch --no-merged

# 提示：此时 feature/payment 是真正的“未合并分支”，可用于测试删除保护逻辑
```

分支图：

![](https://cdn.nlark.com/yuque/0/2025/png/49961487/1764325912138-e74e21aa-99df-46b7-ab5a-884302d623b5.png)

main分支下文件图：

![](https://cdn.nlark.com/yuque/0/2025/png/49961487/1764325930024-74a3006a-2098-47be-b3ba-2c0a196bf2c4.png)

feature/notification下文件图：

![](https://cdn.nlark.com/yuque/0/2025/png/49961487/1764326404795-f2df3a41-cb68-438d-b6ce-d4f5da8c1d2c.png)

feature/user-auth下文件图：

![](https://cdn.nlark.com/yuque/0/2025/png/49961487/1764326425999-6e4fb7ff-3a0e-4180-9e7f-03ab069acd70.png)

feature/payment下文件图：

![](https://cdn.nlark.com/yuque/0/2025/png/49961487/1764326386127-7af052c0-66e4-439f-b4b7-d3ea855264ce.png)
...
![](https://cdn.nlark.com/yuque/0/2025/png/49961487/1764333716349-6c174a37-e1dc-427f-b12c-ce7f9579d90b.png)
...
```powershell
# ======================================================================
# 模拟基于 "Git Flow" 风格的开发流程：
#   - main：主干分支（生产就绪）
#   - develop：集成分支（用于合并所有功能）
#   - feature/*：功能开发分支
# 目标：将两个功能（api-v2 和 dashboard）先合并到 develop，再整体合入 main
# ======================================================================
...
# ------------------------------------------------------------------
# 第 1 步：从 main 创建 develop 集成分支
# ------------------------------------------------------------------
git checkout main                # 切换到主干分支（确保起点干净）
git checkout -b develop          # 基于当前 main 创建并切换到新分支 'develop'

# ------------------------------------------------------------------
# 第 2 步：开发 API v2 功能
# ------------------------------------------------------------------
git checkout -b feature/api-v2   # 从 develop 分支创建新功能分支 'feature/api-v2'

# 创建 src 目录（PowerShell 不会自动创建父目录！）
mkdir -Force src

# 写入新功能文件（注意：这是 untracked 文件，需显式 add）
echo "api v2" > src/api-v2.ts

# 提交更改（-a 会自动包含已跟踪文件的修改，但新文件必须先 add！）
# 由于 api-v2.ts 是全新文件，此处应先 git add（但 -am 在某些 shell 中可能失败）
# 更安全写法（推荐）：
git add src/api-v2.ts
git commit -m "API v2 implementation"
# # 但若你确定文件已被跟踪（或使用 Bash），-am 可能生效
# git commit -am "API v2 implementation"

# 将功能合并回 develop
git checkout develop                             # 切回集成分支
git merge feature/api-v2 -m "Merge branch 'feature/api-v2' into develop"  # 合并并生成合并提交

# ------------------------------------------------------------------
# 第 3 步：开发 Dashboard 功能
# ------------------------------------------------------------------
git checkout -b feature/dashboard                # 从当前 develop（已含 api-v2）创建新功能分支
...
# # 提交（同上，假设文件可被 -a 捕获；更安全做法仍是先 git add）
...
# ------------------------------------------------------------------
# 第 4  步：将完整的 develop 集成成果发布到 main
# ------------------------------------------------------------------
git checkout main                                # 切回主干
git merge develop -m "Merge branch 'develop' into main"  # 将所有功能一次性合入 main

# ======================================================================
# ✅ 最终状态：
#   - main 包含：README.md（如有）、src/api-v2.ts、src/dashboard.ts
#   - develop 与 main 指向同一提交（快进或合并提交）
#   - feature/api-v2 和 feature/dashboard 仍存在（可后续删除）
# ======================================================================
```
...
![](https://cdn.nlark.com/yuque/0/2025/png/49961487/1764334431870-77fff11d-b721-437c-a5dd-aca89fd2c9e7.png)
...
```powershell
# ======================================================================
# 模拟两位开发者（Alice 和 Bob）并行开发不同功能：
#   - Alice 开发登录功能（login）
#   - Bob 开发个人资料功能（profile）
# 目标：
#   ✅ Alice 的功能已合并到 main
#   ⚠️ Bob 的功能尚未合并（用于测试“未合并分支保护”等逻辑）
# 注意：两人均从同一个 main 起点独立开发，无交叉依赖
# ======================================================================

# ------------------------------------------------------------------
# 第 1 步：Alice 创建她的功能分支并提交代码
# ------------------------------------------------------------------
git checkout -b alice/feature-login    # 从当前 main 创建并切换到 Alice 的分支

# 创建 src 目录（确保父目录存在）
mkdir -Force src

# 写入 Alice 的新功能文件（注意：这是全新文件）
echo "login by alice" > src/login.ts

# 提交更改
# ⚠️ 注意：src/login.ts 是 untracked 文件，严格来说应先 `git add`
# 但在某些 shell（如 Bash）中，若目录已存在，-am 可能生效；
# 为保险起见，生产脚本建议显式 add
git commit -am "Implement login (Alice)"


# ------------------------------------------------------------------
# 第 2 步：Bob 从原始 main 开始开发（关键：切回 main！）
# ------------------------------------------------------------------
git checkout main                      # 切回 main —— 确保 Bob 的分支不包含 Alice 的代码！

git checkout -b bob/feature-profile    # 从干净的 main 创建 Bob 的分支
...
# ------------------------------------------------------------------
# 第 3 步：Alice 先将她的功能合并到 main
# ------------------------------------------------------------------
git checkout main                      # 切回主干
git merge alice/feature-login -m "Merge branch 'alice/feature-login'"
# ✅ 此时 main 包含 login.ts，但不包含 profile.ts


# ------------------------------------------------------------------
# 第 4 步：Bob 的分支保持未合并状态（故意不 merge）
# ------------------------------------------------------------------
# ⚠️ 注意：此处没有执行 `git merge bob/feature-profile`
# → bob/feature-profile 仍是“未合并分支”，可用于：
#    - 测试分支保护策略
#    - 演示“未推送/未集成”状态
#    - 模拟协作中的延迟合并场景
...
# ======================================================================
# ✅ 最终仓库状态验证点：
#   - main 分支包含：src/login.ts（来自 Alice），但没有 src/profile.ts
#   - 执行 `git branch --merged` 应包含：alice/feature-login, main
#   - 执行 `git branch --no-merged` 应包含：bob/feature-profile
#   - 提交图谱显示两条独立分支线（无交叉）
# ======================================================================
```
