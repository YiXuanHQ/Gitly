...
const getGlobalState = () => {
	...
	return {
		...
		customEmojiShortcodeMapping: null as any
	};
};
...
const getInitialState = () => {
	...
	return {
		config: {
			emojiShortcodeMapping: null as any
		}
	};
};

// Placeholder for GG namespace types
interface GitCommit {
	hash: string;
	message: string;
}
...
export class TextFormatter {
	...
	private readonly commits: ReadonlyArray<GitCommit>;
	private issueLinking: IssueLinking | null = null;
	 ...
	 */
	constructor(commits: ReadonlyArray<GitCommit | CommitInfo>, repoIssueLinkingConfig: IssueLinkingConfig | null, config: TF.Config) {
		this.config = Object.assign({ commits: false, emoji: false, issueLinking: false, markdown: false, multiline: false, urls: false }, config);
		this.commits = commits as ReadonlyArray<GitCommit>;
		const issueLinkingConfig = repoIssueLinkingConfig !== null
			? repoIssueLinkingConfig
			: getGlobalState().issueLinkingConfig;
		...
		// 注册自定义 emoji 映射
		const customMappings = getInitialState().config.emojiShortcodeMapping || getGlobalState().customEmojiShortcodeMapping;
		if (customMappings && Array.isArray(customMappings)) {
			TextFormatter.registerCustomEmojiMappings(customMappings);
		}
	}
	 ...
	 */
	public static registerCustomEmojiMappings(mappings: ReadonlyArray<CustomEmojiShortcodeMapping>): void {
 ...
 */
export function isUrlElem(elem: Element): boolean {
 ...
 */
export function isExternalUrlElem(elem: Element): boolean {
 ...
 */
export function isInternalUrlElem(elem: Element): boolean {
 ...
 */
function parseIssueLinkingConfig(issueLinkingConfig: IssueLinkingConfig | null): IssueLinking | null {
