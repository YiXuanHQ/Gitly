...
> 📚 **文档完善与技术栈澄清**：全面完善项目文档，修正技术栈描述，统一版本信息，并更新行为准则至最新标准。

### 文档完善与本地化 📖

- **README 文档大幅完善**：
  - **中文版 (`README.md`)**：
    - 添加详细的项目概览，包括版本号、技术栈、性能指标、语言支持
    - 完善核心功能说明，详细描述 Git 提交图、侧边栏视图、Assistant 面板的每个功能
    - 新增完整的技术架构章节，包括：
      - 核心技术栈说明（前后端技术详细列表）
      - 架构设计（目录结构图）
      - 数据流与通信机制（流程图说明）
      - Git 命令执行层（DataSource 类详解）
      - Git 图渲染引擎（Graph 类渲染流程）
      - Assistant 面板架构（组件化设计）
      - 性能优化策略（4 个方面的优化）
      - 可扩展性设计（配置驱动、消息驱动等）
    - 完善开发指南，包括开发环境搭建、构建命令、测试、调试方法
    - 添加命令速查表、推荐配置、路线图
  - **英文版 (`README_EN.md`)**：
    - 创建完整的英文版本文档，与中文版结构和内容保持一致
    - 修正重复内容，统一文档格式
    - 包含所有技术细节和架构说明
  - **技术栈描述修正**：
    - 移除错误的描述（React、D3.js、simple-git）
    - 明确实际使用的技术：
      - 后端：TypeScript + VS Code Extension API + Node.js child_process
      - 前端：原生 DOM + SVG/Canvas（无框架）
      - Git 操作：直接使用 `child_process.spawn` 执行 Git CLI

- **行为准则更新**：
  - `CODE_OF_CONDUCT.md` 更新为 **Contributor Covenant 2.1** 标准版本
  - 使用官方英文版本，符合国际开源项目标准
  - 包含详细的执行指南（Enforcement Guidelines），涵盖 4 个级别的处理措施
  - 在文档末尾添加中文说明和官方翻译链接

- **贡献指南完善**：
  - `CONTRIBUTING.md` 重写为简体中文
  - 覆盖提 Issue、提 PR、开发环境搭建与调试流程等完整内容

### 版本信息统一 🔖

- **版本号统一**：
  - 将 VSIX 扩展版本号（`package.json` 中的 `version` 字段）统一调整为 **1.1.0**
  - 所有文档中的版本号、发布日期统一更新为 **1.1.0 / 2025-12-19**

- **VS Code 版本要求更新**：
  - 从 `^1.80.0` 更新为 `^1.107.0`，与最新 VS Code API 保持一致
  - 更新所有文档中的版本要求说明
...
- **统一文档格式**：
  - 所有文档使用统一的 Markdown 格式和样式
  - 添加项目徽章（版本、许可证、VS Code 版本）
  - 统一章节结构和编号方式

- **完善文档链接**：
  - 修正文档间的交叉引用链接
  - 确保所有链接指向正确的文件或章节
  - 添加 GitHub Releases 链接
...
- **架构文档**：
  - 详细说明 Extension Host 和 Webview 的架构
  - 解释数据流和通信机制
  - 说明各个核心类的职责和实现
...
- **API 文档**：
  - 在 README 中补充主要 API 方法说明
  - 说明配置选项和使用方法

### 已知问题修正 ✅

- **文档中的技术栈描述**：
  - 修正了之前文档中关于 React、D3.js、simple-git 的错误描述
  - 明确说明实际使用的技术栈
...
1. **文档更新**（推荐）：
   - 建议查看更新后的 README，了解详细的技术架构和功能说明
   - 查看新的 CODE_OF_CONDUCT.md，了解社区行为准则

2. **功能兼容性**：
   - 所有 v1.0.2 的功能在 v1.1.0 中完全兼容
   - 本次更新主要为文档完善，不涉及功能变更

3. **VS Code 版本要求**：
   - 确保 VS Code 版本 >= 1.107.0
   - 如果使用较旧版本，请先升级 VS Code

4. **配置兼容性**：
   - 所有现有配置在 v1.1.0 中完全兼容
   - 无需修改任何配置
