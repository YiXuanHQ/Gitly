...
export class CommandManager extends Disposable {
	...
	constructor(context: vscode.ExtensionContext, avatarManger: AvatarManager, dataSource: DataSource, extensionState: ExtensionState, repoManager: RepoManager, gitExecutable: GitExecutable | null, onDidChangeGitExecutable: Event<GitExecutable>, logger: Logger) {
		...
		this.registerCommand('gitly.view', (arg) => this.view(arg));
		this.registerCommand('gitly.addGitRepository', () => this.addGitRepository());
		this.registerCommand('gitly.removeGitRepository', () => this.removeGitRepository());
		this.registerCommand('gitly.clearAvatarCache', () => this.clearAvatarCache());
		this.registerCommand('gitly.fetch', () => this.fetch());
		this.registerCommand('gitly.endAllWorkspaceCodeReviews', () => this.endAllWorkspaceCodeReviews());
		this.registerCommand('gitly.endSpecificWorkspaceCodeReview', () => this.endSpecificWorkspaceCodeReview());
		this.registerCommand('gitly.resumeWorkspaceCodeReview', () => this.resumeWorkspaceCodeReview());
		this.registerCommand('gitly.version', () => this.version());
		this.registerCommand('gitly.openFile', (arg) => this.openFile(arg));
		this.registerCommand('gitly.openSettings', () => this.openSettings());
	...
	/**
	 * The method run when the `gitly.view` command is invoked.
	...
	/**
	 * The method run when the `gitly.addGitRepository` command is invoked.
	 */
	...
	/**
	 * The method run when the `gitly.removeGitRepository` command is invoked.
	 */
	...
	/**
	 * The method run when the `gitly.clearAvatarCache` command is invoked.
	 */
	...
	/**
	 * The method run when the `gitly.fetch` command is invoked.
	 */
	private fetch() {
		const repos = this.repoManager.getRepos();
	...
	/**
	 * The method run when the `gitly.endAllWorkspaceCodeReviews` command is invoked.
	 */
	...
	/**
	 * The method run when the `gitly.endSpecificWorkspaceCodeReview` command is invoked.
	 */
	...
	/**
	 * The method run when the `gitly.resumeWorkspaceCodeReview` command is invoked.
	 */
	...
	/**
	 * The method run when the `gitly.version` command is invoked.
	 */
	 ...
	 * The method run when the `gitly.openFile` command is invoked.
	...
	/**
	 * Open the Gitly Extension Settings.
	 */
	private openSettings() {
		openExtensionSettings().then((errorInfo) => {
			if (errorInfo !== null) {
				showErrorMessage(errorInfo);
			}
		});
