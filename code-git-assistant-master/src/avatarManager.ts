...
export class AvatarManager extends Disposable {
	...
	private fetchFromGithub(avatarRequest: AvatarRequestItem, owner: string, repo: string) {
		...
		https.get({
			...
			headers: { 'User-Agent': 'vscode-gitly' },
			...
			res.on('end', async () => {
				...
				if (res.statusCode === 200) { // Success
					...
					if (commit.author && commit.author.avatar_url) { // Avatar url found
						let img = await this.downloadAvatarImage(avatarRequest.email, this.withAvatarSize(commit.author.avatar_url, 162));
						if (img !== null) {
	...
	/**
	 * Add a `size` parameter to an avatar url.
	 * - If the url already has a query string, use `size=...` in query params.
	 * - If it has no query string (as in some test fixtures), keep the legacy `/&size=...` path form.
	 */
	private withAvatarSize(avatarUrl: string, size: number): string {
		try {
			...
			// Legacy fallback used by older logic/tests: append `&size=...` as a path segment.
			if (!u.pathname.endsWith('/')) u.pathname += '/';
			u.pathname += `&size=${size}`;
			return u.toString();
		} catch {
