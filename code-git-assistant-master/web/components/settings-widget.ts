/**
 * 从 vscode-git-graph-develop 迁移的设置组件
 */
...
} from '../utils/vscode-git-utils.js';
import { Dialog, DialogInputType, DialogInput, DialogInputValue } from './dialog.js';
import { TargetType } from './context-menu.js';

// Create a singleton dialog instance
...
// Placeholder interfaces - these should be provided by the application
interface GitGraphView {
	getRepoState(repo: string): any;
	getRepoConfig(): any;
	isConfigLoading(): boolean;
	getBranchOptions(): Array<{ name: string; value: string }>;
	getBranches(): string[];
	saveRepoStateValue(repo: string, key: string, value: any): void;
...
// Placeholder for globalState and initialState
const getGlobalState = () => {
	if (typeof (window as any).globalState !== 'undefined') {
		return (window as any).globalState;
	}
	return {
		issueLinkingConfig: null as any
	};
};

const getInitialState = () => {
	if (typeof (window as any).initialState !== 'undefined') {
		return (window as any).initialState;
	}
	return {
		config: {
			dialogDefaults: {
				...
				customPullRequestProviders: [] as any[]
			}
		}
	};
};

const updateGlobalViewState = (key: string, value: any): void => {
	if (typeof (window as any).updateGlobalViewState === 'function') {
		(window as any).updateGlobalViewState(key, value);
	}
};

const sendMessage = (msg: any): void => {
	if (typeof (window as any).vscode !== 'undefined' && (window as any).vscode.postMessage) {
		(window as any).vscode.postMessage(msg);
	}
};

const runAction = (action: any, actionName: string): void => {
	if (typeof (window as any).runAction === 'function') {
		(window as any).runAction(action, actionName);
	} else {
		sendMessage(action);
	}
};
...
function getOnRepoLoadShowCheckedOutBranch(value: any): boolean {
	return value === true || value === 1 || value === 'enabled';
}

function getOnRepoLoadShowSpecificBranches(value: any): string[] {
	return Array.isArray(value) ? value : [];
}

function getShowStashes(value: any): boolean {
	return value === true || value === 1 || value === 'enabled';
}

function getShowTags(value: any): boolean {
	return value === true || value === 1 || value === 'enabled';
}

function getIncludeCommitsMentionedByReflogs(value: any): boolean {
	return value === true || value === 1 || value === 'enabled';
}

function getOnlyFollowFirstParent(value: any): boolean {
	return value === true || value === 1 || value === 'enabled';
}

// Types for Git Graph
enum BooleanOverride {
	Default = 0,
	Enabled = 1,
	Disabled = 2
}
...
interface GitRepoState {
	name: string | null;
	onRepoLoadShowCheckedOutBranch: any;
	onRepoLoadShowSpecificBranches: string[] | null;
	showStashes: any;
	showTags: any;
	includeCommitsMentionedByReflogs: any;
	onlyFollowFirstParent: any;
	hideRemotes: string[];
...
interface PullRequestConfig {
	...
	destBranch: string;
	custom: { name: string; templateUrl: string } | null;
}
...
export class SettingsWidget {
	...
	private repo: GitRepoState | null = null;
	private config: GitRepoConfig | null = null;
	...
	private render() {
		if (this.currentRepo !== null && this.repo !== null) {
			...
			const initialBranchesLocallyConfigured = this.repo.onRepoLoadShowCheckedOutBranch !== BooleanOverride.Default || this.repo.onRepoLoadShowSpecificBranches !== null;
			...
			if (this.config !== null) {
				...
				if (pullRequestConfig !== null) {
					const provider = escapeHtml((pullRequestConfig.provider === PullRequestProvider.Bitbucket
						? 'Bitbucket'
						: pullRequestConfig.provider === PullRequestProvider.Custom
							? (pullRequestConfig.custom?.name || 'Custom')
							: pullRequestConfig.provider === PullRequestProvider.GitHub
								? 'GitHub'
								: 'GitLab'
			...
			document.getElementById('editRepoName')!.addEventListener('click', () => {
				...
				getDialog().showForm('Specify a Name for this Repository:', [
			...
			if (this.repo.name !== null) {
				document.getElementById('deleteRepoName')!.addEventListener('click', () => {
					...
					getDialog().showConfirmation('Are you sure you want to delete the manually configured name <b><i>' + escapeHtml(this.repo.name) + '</i></b> for this repository, and use the default name from the File System <b><i>' + escapeHtml(getRepoName(this.currentRepo)) + '</i></b>?', 'Yes, delete', () => {
						if (this.currentRepo === null) return;
			...
			document.getElementById('editInitialBranches')!.addEventListener('click', () => {
				...
				getDialog().showForm('<b>Configure Initial Branches</b><p style="margin:6px 0;">Configure the branches that are initially shown when this repository is loaded in the Git Graph View.</p><p style="font-size:12px; margin:6px 0 0 0;">Note: When "Checked Out Branch" is Disabled, and no "Specific Branches" are selected, all branches will be shown.</p>', [
					...
					if (showCheckedOutBranch !== values[0] || !arraysStrictlyEqualIgnoringOrder(showSpecificBranches, <string[]>values[1])) {
						this.view.saveRepoStateValue(this.currentRepo, 'onRepoLoadShowCheckedOutBranch', values[0] ? BooleanOverride.Enabled : BooleanOverride.Disabled);
			...
			if (initialBranchesLocallyConfigured) {
				document.getElementById('clearInitialBranches')!.addEventListener('click', () => {
					getDialog().showConfirmation('Are you sure you want to clear the branches that are initially shown when this repository is loaded in the Git Graph View?', 'Yes, clear', () => {
						if (this.currentRepo === null) return;
						this.view.saveRepoStateValue(this.currentRepo, 'onRepoLoadShowCheckedOutBranch', BooleanOverride.Default);
			...
			showStashesElem.addEventListener('change', () => {
				...
				if (elem === null) return;
				this.view.saveRepoStateValue(this.currentRepo, 'showStashes', elem.checked ? BooleanOverride.Enabled : BooleanOverride.Disabled);
			...
			showTagsElem.addEventListener('change', () => {
				...
				if (elem === null) return;
				this.view.saveRepoStateValue(this.currentRepo, 'showTags', elem.checked ? BooleanOverride.Enabled : BooleanOverride.Disabled);
				this.view.refresh(true);
			});
			...
			includeCommitsMentionedByReflogsElem.addEventListener('change', () => {
				...
				if (elem === null) return;
				this.view.saveRepoStateValue(this.currentRepo, 'includeCommitsMentionedByReflogs', elem.checked ? BooleanOverride.Enabled : BooleanOverride.Disabled);
				this.view.refresh(true);
			});
			...
			settingsOnlyFollowFirstParentElem.addEventListener('change', () => {
				...
				if (elem === null) return;
				this.view.saveRepoStateValue(this.currentRepo, 'onlyFollowFirstParent', elem.checked ? BooleanOverride.Enabled : BooleanOverride.Disabled);
				this.view.refresh(true);
			});
			...
			if (this.config !== null) {
				document.getElementById('editUserDetails')!.addEventListener('click', () => {
					...
					const userName = this.config.user.name, userEmail = this.config.user.email;
					getDialog().showForm('Set the user name and email used by Git to record the Author and Committer of commit objects:', [
						...
						runAction({
							...
							location: useGlobally ? GitConfigLocation.Global : GitConfigLocation.Local,
				...
				if (userNameSet || userEmailSet) {
					document.getElementById('removeUserDetails')!.addEventListener('click', () => {
						...
						getDialog().showConfirmation('Are you sure you want to remove the <b>' + (isGlobal ? 'globally' : 'locally') + ' configured</b> user name and email, which are used by Git to record the Author and Committer of commit objects?', 'Yes, remove', () => {
							if (this.currentRepo === null) return;
							runAction({
								...
								location: isGlobal ? GitConfigLocation.Global : GitConfigLocation.Local
				...
				document.getElementById('settingsAddRemote')!.addEventListener('click', () => {
					getDialog().showForm('Add a new remote to this repository:', [
				...
				addListenerToClass('editRemote', 'click', (e) => {
					...
					getDialog().showForm('Edit the remote <b><i>' + escapeHtml(remote.name) + '</i></b>:', [
				...
				addListenerToClass('deleteRemote', 'click', (e) => {
					const remote = this.getRemoteForBtnEvent(e);
					if (remote === null) return;
					getDialog().showConfirmation('Are you sure you want to delete the remote <b><i>' + escapeHtml(remote.name) + '</i></b>?', 'Yes, delete', () => {
						if (this.currentRepo === null) return;
				...
				addListenerToClass('fetchRemote', 'click', (e) => {
					...
					getDialog().showForm('Are you sure you want to fetch from the remote <b><i>' + escapeHtml(remote.name) + '</i></b>?', [
				...
				addListenerToClass('pruneRemote', 'click', (e) => {
					const remote = this.getRemoteForBtnEvent(e);
					if (remote === null) return;
					getDialog().showConfirmation('Are you sure you want to prune remote-tracking references that no longer exist on the remote <b><i>' + escapeHtml(remote.name) + '</i></b>?', 'Yes, prune', () => {
						if (this.currentRepo === null) return;
			...
			if (this.repo.issueLinkingConfig !== null || globalState2.issueLinkingConfig !== null) {
				document.getElementById('removeIssueLinking')!.addEventListener('click', () => {
					if (this.repo === null) return;
					const globalState3 = getGlobalState();
					const locallyConfigured = this.repo.issueLinkingConfig !== null;
					getDialog().showConfirmation('Are you sure you want to remove ' + (locallyConfigured ? (globalState3.issueLinkingConfig !== null ? 'the <b>locally configured</b> ' : '') + 'Issue Linking from this repository' : 'the <b>globally configured</b> Issue Linking in Git Graph') + '?', 'Yes, remove', () => {
			...
			if (this.config !== null) {
				document.getElementById('editPullRequestIntegration')!.addEventListener('click', () => {
					...
					if (this.config.remotes.length === 0) {
						getDialog().showError('Unable to configure the "Pull Request Creation" Integration', 'The repository must have at least one remote to configure the "Pull Request Creation" Integration. There are no remotes in the current repository.', null, null);
						return;
					}

					type DeepWriteable<T> = { -readonly [P in keyof T]: DeepWriteable<T[P]> };
					let config: DeepWriteable<PullRequestConfig>;
				...
				if (this.repo.pullRequestConfig !== null) {
					document.getElementById('removePullRequestIntegration')!.addEventListener('click', () => {
						getDialog().showConfirmation('Are you sure you want to remove the configured "Pull Request Creation" Integration?', 'Yes, remove', () => {
			...
			document.getElementById('exportRepositoryConfig')!.addEventListener('click', () => {
				getDialog().showConfirmation('Exporting the Git Graph Repository Configuration will generate a file that can be committed in this repository. It allows others working in this repository to use the same configuration.', 'Yes, export', () => {
					if (this.currentRepo === null) return;
	 ...
	 */
	private setIssueLinkingConfig(config: IssueLinkingConfig | null, global: boolean) {
		if (this.currentRepo === null || this.repo === null) return;
	 ...
	 */
	private setPullRequestConfig(config: PullRequestConfig | null) {
		if (this.currentRepo === null) return;
	...
	private showIssueLinkingDialog(defaultIssueRegex: string | null, defaultIssueUrl: string | null, defaultUseGlobally: boolean, isEdit: boolean) {
		...
		if (!isEdit && defaultIssueRegex === null && defaultIssueUrl === null) {
			// Note: getCommits() may not exist on GitGraphView, using placeholder
			const commits = (this.view as any).getCommits ? (this.view as any).getCommits() : [];
			defaultIssueRegex = SettingsWidget.autoDetectIssueRegex(commits);
		...
		getDialog().showForm(html, [
			...
			if (regExpParseError !== null) {
				getDialog().showError('Invalid Issue Regex', regExpParseError, 'Go Back', () => {
					this.showIssueLinkingDialog(issueRegex, issueUrl, useGlobally, isEdit);
				});
			} else if (!(/\$([1-9][0-9]*)/.test(issueUrl))) {
				getDialog().showError('Invalid Issue URL', 'The Issue URL does not contain any placeholders ($1, $2, etc.) for the issue number components captured in the Issue Regex.', 'Go Back', () => {
					this.showIssueLinkingDialog(issueRegex, issueUrl, useGlobally, isEdit);
				});
			} else {
	 ...
	 */
	private showCreatePullRequestIntegrationDialog1(config: DeepWriteable<PullRequestConfig>) {
		if (this.config === null) return;
		...
		const providerOptions = [
			{ name: 'Bitbucket', value: (PullRequestProvider.Bitbucket).toString() },
			{ name: 'GitHub', value: (PullRequestProvider.GitHub).toString() },
			{ name: 'GitLab', value: (PullRequestProvider.GitLab).toString() }
		];
		...
		if (config.provider === PullRequestProvider.Custom) {
		...
		getDialog().showForm('Configure "Pull Request Creation" Integration (Step&nbsp;1/2)', [
			{
			...
			let newProvider = <PullRequestProvider>parseInt(<string>values[0]);
			if (newProvider > 3) newProvider = PullRequestProvider.Custom;
			...
			if (newProvider === PullRequestProvider.Custom) {
			...
			if (config.provider !== PullRequestProvider.GitLab || config.destRemote !== newDestRemote) {
	 ...
	 * @param config The pull request configuration.
	 */
	private showCreatePullRequestIntegrationDialog2(config: DeepWriteable<PullRequestConfig>) {
		if (this.config === null) return;
		...
		const updateConfigWithFormValues = (values: any[]) => {
			...
			config.destProjectId = config.provider === PullRequestProvider.GitLab ? <string>values[5] : '';
			const destBranch = <string>values[config.provider === PullRequestProvider.GitLab ? 6 : 5];
		...
		if (config.provider === PullRequestProvider.GitLab) {
		...
		getDialog().showForm('Configure "Pull Request Creation" Integration (Step&nbsp;2/2)', inputs, 'Save Configuration', (values: any[]) => {
			updateConfigWithFormValues(values);
			this.setPullRequestConfig(config);
		}, { type: TargetType.Repo }, 'Back', (values: any[]) => {
			updateConfigWithFormValues(values);
			this.showCreatePullRequestIntegrationDialog1(config);
		});
	}
	 ...
	 */
	private static autoDetectIssueRegex(commits: ReadonlyArray<{ message: string }>) {
