...
| 🧬 Git 视图表 | Git 提交/分支关系与合流路径 | Git Graph 渲染器 |
| ☁️ 远程仓库 | 添加/重命名/更新/删除远程 | Web 组件 + VS Code 命令 |
| 🌿 分支管理 | 分支树、创建/切换/合并 | Web 组件 |
| 🏷️ 标签管理 | 创建注释/轻量标签、推送/删除 | Web 组件 |
| ⚠️ 冲突解决 | 冲突检测、三栏对比编辑、解决指令 | Web 组件 |
...
├── services/           # Git 服务封装
├── providers/          # 树视图提供者
├── webview/            # Webview 容器（加载 media/ 中的前端资源）
├── utils/              # 工具
└── types/              # 类型

web/                    # Webview 前端源码（TS + DOM/Canvas/SVG）
├── app.ts
├── components/
├── styles/
├── types/
...
**项目状态：✅ v1.0.2 正式版** | **最后更新：2025-12-11**
