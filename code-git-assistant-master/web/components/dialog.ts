...
import {
	...
	addListenerToCollectionElems,
	REF_INVALID_REGEX,
	CLASS_FOCUSSED,
	CLASS_SELECTED,
	EventOverlay
} from '../utils/vscode-git-utils.js';
import { TargetType, RepoTarget } from './context-menu.js';
...
function closeDialogAndContextMenu(): void {
	// Close context menu if available
	const contextMenu = (window as any).contextMenu;
...
const getInitialState = () => {
	...
	return {
		config: {
			dialogDefaults: {
				general: {
					referenceInputSpaceSubstitution: null as string | null
				}
			}
		}
	};
...
export class Dialog {
	 ...
	 */
	public showError(message: string, reason: string | null, actionName: string | null, actioned: (() => void) | null) {
	...
	private render(open: boolean) {
		...
		if (this.open !== open) {
			...
			if (open) {
				...
				addListenerToCollectionElems(this.optionsElem.children, 'mousemove', (e: Event) => {
					if (!e.target) return;
