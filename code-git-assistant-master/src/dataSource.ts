...
export class DataSource extends Disposable {
	...
	constructor(gitExecutable: GitExecutable | null, onDidChangeConfiguration: Event<vscode.ConfigurationChangeEvent>, onDidChangeGitExecutable: Event<GitExecutable>, logger: Logger) {
		...
		this.registerDisposables(
			onDidChangeConfiguration((event) => {
				if (
					event.affectsConfiguration('gitly.date.type') || event.affectsConfiguration('gitly.dateType') ||
					event.affectsConfiguration('gitly.repository.commits.showSignatureStatus') || event.affectsConfiguration('gitly.showSignatureStatus') ||
					event.affectsConfiguration('gitly.repository.useMailmap') || event.affectsConfiguration('gitly.useMailmap')
				) {
	...
	public getCommitComparison(repo: string, fromHash: string, toHash: string): Promise<GitCommitComparisonData> {
		...
		] as const).then(([nameStatus, numStat, status]) => ({
			fileChanges: generateFileChanges(nameStatus, numStat, status),
			error: null
		})).catch((errorMessage) => ({ fileChanges: [], error: errorMessage }));
	}
