...
export class StatusBarItem extends Disposable {
	...
	constructor(initialNumRepos: number, onDidChangeRepos: Event<RepoChangeEvent>, onDidChangeConfiguration: Event<vscode.ConfigurationChangeEvent>, logger: Logger) {
		...
		statusBarItem.command = 'gitly.view';
		...
		this.registerDisposables(
			...
			onDidChangeConfiguration((event) => {
				if (event.affectsConfiguration('gitly.showStatusBarItem') || event.affectsConfiguration('gitly.language')) {
					this.refresh();
				}
			}),
			statusBarItem
	...
	/**
	 * Show or hide the Status Bar Item according to the configured value of `gitly.showStatusBarItem`, and the number of repositories known to Git Graph.
	 */
	private refresh() {
		// Update text and tooltip when language changes
		this.statusBarItem.text = t('statusBar.text');
